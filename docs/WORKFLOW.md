...
```text
Windows desktop: %USERPROFILE%\src\speakeasy
Windows laptop:  %USERPROFILE%\src\speakeasy
...
docs/CODEX_REVIEW_LOOP.md, and git status. Continue the next unchecked task. Do
not ask for secrets in chat. Use existing secret names only, preserve the
documented security constraints, run verification, and report blockers.
```

## Human-Gated Codex PR Loop

Use `docs/CODEX_REVIEW_LOOP.md` as the shared protocol. Joaquim pins a task to
an exact PR head. Codex reviews the current code and discussion, implements and
verifies the authorized scope, pushes a new head when requested, and posts a
structured response. Optional outside reviews are advisory inputs that Codex
evaluates on their evidence.

Keep the loop human-gated until branch protection, read-only default Action
permissions, a least-privileged automation identity, exact-head/idempotency
checks, one-writer concurrency, and a manual disable switch are in place.
