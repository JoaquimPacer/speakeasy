...
final class KithraPlaintextTempFileJanitor: @unchecked Sendable {
    ...
    func preserveWhileInUse(_ url: URL) {
        lock.lock()
        activePaths.insert(canonicalPath(for: url))
        lock.unlock()
    }

    func release(_ url: URL) {
        lock.lock()
        activePaths.remove(canonicalPath(for: url))
        lock.unlock()
    }
    ...
    private func cleanupDirectChildren(
        ...
        for child in children where matches(child.lastPathComponent) {
            ...
            lock.lock()
            let isActive = activePaths.contains(canonicalPath(for: child))
            if !isActive {
                do {
...
final class DefaultMediaPipeline: MediaPipelining {
    ...
    func makeThumbnail(
        ...
        plaintextTempJanitor.preserveWhileInUse(stagedURL)
